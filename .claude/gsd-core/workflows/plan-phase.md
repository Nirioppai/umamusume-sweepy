...
@C:/Active Codebase/umamusume-sweepy/.claude/gsd-core/references/revision-loop.md
@C:/Active Codebase/umamusume-sweepy/.claude/gsd-core/references/gate-prompts.md
@C:/Active Codebase/umamusume-sweepy/.claude/gsd-core/references/agent-contracts.md
@C:/Active Codebase/umamusume-sweepy/.claude/gsd-core/references/gates.md
</required_reading>
...
When `TDD_MODE` is `true`, the planner agent is instructed to apply `type: tdd` to eligible tasks using heuristics from `references/tdd.md`. The planner's `<required_reading>` is extended to include `@C:/Active Codebase/umamusume-sweepy/.claude/gsd-core/references/tdd.md` so gate enforcement rules are available during planning.
...
- Planner is instructed to produce `SKELETON.md` in the phase directory alongside `PLAN.md`. The template lives at `@C:/Active Codebase/umamusume-sweepy/.claude/gsd-core/references/skeleton-template.md`.
...
**If found:**
1. Read template: `C:/Active Codebase/umamusume-sweepy/.claude/gsd-core/templates/VALIDATION.md`
...
UI_GATE_JS=$(for _c in "$_GSD_RT/gsd-core/bin/lib/ui-safety-gate.cjs" "$_GSD_RT/bin/lib/ui-safety-gate.cjs" "$_GSD_RT/.claude/bin/lib/ui-safety-gate.cjs" "C:/Active Codebase/umamusume-sweepy/.claude/gsd-core/bin/lib/ui-safety-gate.cjs" "C:/Active Codebase/umamusume-sweepy/.claude/bin/lib/ui-safety-gate.cjs"; do [ -f "$_c" ] && { echo "$_c"; break; }; done)
...
${TDD_MODE === 'true' ? `
<tdd_mode_active>
**TDD Mode is ENABLED.** Apply TDD heuristics from @C:/Active Codebase/umamusume-sweepy/.claude/gsd-core/references/tdd.md to all eligible tasks:
...
**MVP_MODE:** ${MVP_MODE} (when true, follow vertical-slice rules from `@C:/Active Codebase/umamusume-sweepy/.claude/gsd-core/references/planner-mvp-mode.md`; when false, ignore MVP guidance entirely.)
...
${MVP_MODE === 'true' ? `
<mvp_mode_active>
**MVP Mode is ENABLED.** Follow vertical-slice planning rules from @C:/Active Codebase/umamusume-sweepy/.claude/gsd-core/references/planner-mvp-mode.md. Each plan must deliver a complete vertical slice — thin end-to-end functionality rather than horizontal layers.
</mvp_mode_active>
` : ''}
</planning_context>
